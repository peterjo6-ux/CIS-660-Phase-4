-- =============================================================================
-- STEP 5 — SQL Analytics Pack (10 queries)
-- Demonstrates: JOINs, GROUP BY, HAVING, subqueries, CASE, ranking
-- =============================================================================


-- QUERY 1 — Multi-table JOIN + GROUP BY + ORDER BY
-- KPI: Top 10 companies by total complaint volume
...
-- KPI: Products that received more than 500 complaints
...
-- QUERY 3 — Subquery
-- KPI: Companies whose complaint count exceeds the overall average
SELECT co.company_name,
       COUNT(c.complaint_id) AS complaint_count
FROM   complaints c
JOIN   companies co ON c.company_id = co.company_id
...
-- QUERY 4 — CASE logic
-- KPI: Classify companies into complaint volume tiers
SELECT co.company_name,
...
-- KPI: Top 3 issues within each product category
SELECT *
FROM (
    SELECT p.product_name,
           i.issue_name,
...
-- KPI: Monthly complaint volume with running year-to-date total
SELECT month_start,
       monthly_count,
...
-- KPI: States with > 1000 complaints where dispute rate exceeds 20%
SELECT c.state,
       COUNT(c.complaint_id) AS total_complaints,
...
-- QUERY 9 (BONUS) — Correlated subquery
-- KPI: For each product, the company with the most complaints
SELECT p.product_name,
       co.company_name,
...
HAVING COUNT(c.complaint_id) = (
           SELECT MAX(cnt)
           FROM  (SELECT COUNT(c2.complaint_id) AS cnt
                  FROM   complaints c2
                  WHERE  c2.product_id = c.product_id
                  GROUP  BY c2.company_id) sub
       )
ORDER  BY complaints DESC;


-- QUERY 10 (BONUS) — 3-table JOIN full picture
-- KPI: Complaint resolution snapshot per product-issue pair
SELECT p.product_name,
       i.issue_name,
       COUNT(c.complaint_id)                                        AS total,
